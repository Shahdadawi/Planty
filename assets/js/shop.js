...
function renderPlants(data) {
    ...
    currentPlants.forEach((plant, index) => {
        ...
        viewButton.href = `plant-details.html?latinName=${encodeURIComponent(plant["Latin name"])}`; // بعتنا الاسم اللاتيني عشان نستخدمه لنلاقي النبتة المحددة
...
document.getElementById("searchBtn").addEventListener("click", async () => {
    ...
    try {
      ...
      const response = await fetch(`https://house-plants2.p.rapidapi.com/category/${encodeURIComponent(keyword)}`, {
        method: "GET",
        headers: {
          "X-RapidAPI-Key": "5bda55fb28msh7505d5d7a16332fp1186d4jsn45f4eaba70db",
          "X-RapidAPI-Host": "house-plants2.p.rapidapi.com"
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        plantList.innerHTML = "<p class='text-muted text-center'>No plants found for this category.</p>";
        pagination.innerHTML = "";
      }
  
    } catch (err) {
      console.error("Search error:", err);
      plantList.innerHTML = "<p class='text-danger text-center'>Something went wrong during search.</p>";
      pagination.innerHTML = "";
    } finally {
      loader.style.display = "none";
    }
